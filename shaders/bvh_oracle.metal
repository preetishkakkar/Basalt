// Batched software-BVH oracle. The host supplies the same independent ray corpus used
// for hardware ray queries and compares the results with double-precision world-space
...
kernel void bvh_oracle(const device OracleRay* rays [[buffer(0)]],
                       ...
                       const device float4* bvhNodes [[buffer(7)]],
  ...
  // Production paths offset their origins and use tMin zero. Shifting here exercises an
  // arbitrary finite lower extent without changing the shared traversal signature.
  ...
  PtHit hit = ptTraceBvh(bvhNodes, bvhTriangles, PT_SCENE_ARGS, PT_TEXTURE_ARGS,
