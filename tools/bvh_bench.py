...
Each frame traces --spf samples (4 by default): with one, a frame is too little work to keep the
GPU busy and the driver drops its clock partway through some runs (RTX 4090: 2520 to about 1260
MHz, the last frames' trace time from 0.99 to 1.8 ms), while four hold it within 0.5%. The
graphics clock is logged during every run (nvidia-smi) and the table reports its lowest value
over each combination's timed runs' second halves, so a run that lost it shows.

With --animate the scene moves every frame (--spp counts frames; --spf defaults to 1) and the
table adds the per-frame BVH update's wall and GPU time (median of the last quarter of the
frames); a GPU parallel LBVH refits and rebuilds its TLAS, the others (and --bvh-update rebuild)
rebuild.
...
SCENES = {
    ...
    # tools/fetch_scenes.py makes these (not in the default set: large downloads).
    "conference": ["assets/conference.glb", "assets/court.hdr"],
    "hairball": ["assets/hairball.glb", "assets/court.hdr"],
    "bistro": ["assets/bistro.glb", "assets/court.hdr"],
}
DEFAULT_SCENES = ["materials", "raycone", "manylights", "sponza", "helmet"]
# Views other than the automatic framing (--view YAW PITCH DISTANCE-SCALE): inside the
# conference room (closed and without lights: dark, but every ray traverses it), and Bistro's
# streets from above rather than the whole block.
VIEWS = {
    "conference": ["40", "-5", "0.08"],
    "bistro": ["30", "20", "0.25"],
}
BUILDERS = ["cpu", "gpu-serial", "gpu-lbvh", "gpu-apetrei", "gpu-batched", "gpu-ploc", "gpu-plocpp", "gpu-hploc", "gpu-sah"]
...
class ClockLog:
    """nvidia-smi's graphics clock every 50 ms, for the whole bench (none when it is missing)."""

    def __init__(self, path):
        self.path = path
        try:
            self.process = subprocess.Popen(["nvidia-smi", "--query-gpu=timestamp,clocks.gr", "--format=csv,noheader,nounits",
                                             "-lms", "50"], stdout=open(path, "w"), stderr=subprocess.DEVNULL)
        except OSError:
            self.process = None

    def lowest(self, start, end):
        """The lowest graphics clock (MHz) sampled in the second half of [start, end] (epoch seconds)."""
        if not self.process:
            return None
        lowest = None
        for line in Path(self.path).read_text().splitlines():
            stamp, _, clock = line.partition(",")
            try:
                whole, _, fraction = stamp.strip().partition(".")
                t = time.mktime(time.strptime(whole, "%Y/%m/%d %H:%M:%S")) + float("0." + (fraction or "0"))
                if (start + end) / 2 <= t <= end:
                    lowest = min(lowest, float(clock)) if lowest is not None else float(clock)
            except ValueError:
                continue
        return lowest
...
def run(exe, scene, builder, width, args, output, profile, clocks=None):
    view = ["--view", *VIEWS[scene]] if scene in VIEWS else []
    command = [str(exe), *[str(ROOT / part) for part in SCENES[scene]], *view, "--renderer", "gpu-bvh",
               "--bvh-builder", builder, "--bvh-width", width, "--path-execution", args.execution,
               "--bvh-update", args.bvh_update, "--animate", args.animate, "--bvh-split-clipping", args.split_clipping,
               *(["--bvh-traversal", args.traversal] if width == "binary" else []),
               "--size", args.size, "--spp", str(args.spp), "--spf", str(args.spf), "--seed", "11", "--no-ui", "--no-vsync",
    ...
    if result.returncode or not metadata.exists():
        # A rejected combination, or a tree the traversal cannot take (its depth): recorded, not fatal.
        rejection = [line for line in (text + result.stderr).splitlines()
                     if "require" in line or "unknown BVH builder" in line or "build failed" in line
                     or "needs a traversal stack" in line]
        if rejection:
    ...
    lowest = clocks.lowest(started, finished) if clocks else None
    if lowest is not None:
        values["clock_lowest_mhz"] = lowest
...
def main():
    ...
    parser.add_argument("--scenes", default=",".join(DEFAULT_SCENES), help=f"of {', '.join(SCENES)}")
    ...
    parser.add_argument("--spf", type=int, default=0, help="samples per frame (default 4, 1 with --animate)")
    ...
    parser.add_argument("--split-clipping", default="off", help="early split clipping for every build: off, on or a factor")
    parser.add_argument("--traversal", default="stack", help="the binary layout's --bvh-traversal")
    parser.add_argument("--output", type=Path, default=ROOT / "build" / "bench" / "bvh")
    args = parser.parse_args()
    if args.spf <= 0:
        args.spf = 1 if args.animate != "off" else 4
    args.output.mkdir(parents=True, exist_ok=True)
    clocks = ClockLog(args.output / "clocks.csv")
    report = {"started": time.strftime("%Y-%m-%d %H:%M:%S"), "exe": str(args.exe.resolve()), "size": args.size,
              "spp": args.spp, "spf": args.spf, "repeats": args.repeats, "profile_runs": args.profile_runs,
              "execution": args.execution, "animate": args.animate, "bvh_update": args.bvh_update,
              "split_clipping": args.split_clipping, "traversal": args.traversal,
              "results": [], "skipped": []}
    for scene in args.scenes.split(","):
        ...
        for repeat in range(args.repeats):
            for builder, width, label in (combos if repeat % 2 == 0 else combos[::-1]):
                values, device, revision = run(args.exe, scene, builder, width, args,
                                               args.output / f"{label}-{repeat}.pfm", False, clocks)
        ...
        for builder, width, label in combos:
            lowest = [values["clock_lowest_mhz"] for values in plain[label] if "clock_lowest_mhz" in values]
            row = {"scene": scene, "builder": builder, "width": width, "device": device, "revision": revision,
                   ...
                   "clock_lowest_mhz": min(lowest) if lowest else None, "runs": plain[label]}
    ...
    header = ["scene", "builder", "width", "build ms", "GPU build ms", "collapse ms", "SAH BLAS", "SAH TLAS", "layout SAH",
              ...
              "shadow ms"] + [name[len("bvh_stage_ms_"):] + " ms" for name in stage_names] + ["lowest MHz"]
    ...
    for row in report["results"]:
        ...
        cells.append(f"{row['clock_lowest_mhz']:.0f}" if row["clock_lowest_mhz"] is not None else "-")
        lines.append("| " + " | ".join(cells) + " |")
    table = "\n".join(lines)
    (args.output / "report.md").write_text(table + "\n", encoding="utf-8")
    devices = sorted({row["device"] for row in report["results"]})
    print(f"\nDevice: {', '.join(devices)}; {args.repeats} repeats (median), {args.size}, {args.spp} spp, {args.spf} spf, "
          f"{args.execution}, animate {args.animate}, update {args.bvh_update}, split clipping {args.split_clipping}, traversal {args.traversal}\n")
    print(table)
